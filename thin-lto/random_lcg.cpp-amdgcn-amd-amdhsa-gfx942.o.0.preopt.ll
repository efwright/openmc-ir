; ModuleID = 'random_lcg.cpp-amdgcn-amd-amdhsa-gfx942.o.0.preopt.bc'
...
; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(none)
define hidden noundef range(i64 0, -9223372036854775808) i64 @_ZN6openmc11future_seedEmm(i64 noundef %n, i64 noundef %seed) local_unnamed_addr #1 {
entry:
  %and = and i64 %n, 9223372036854775807
  %cmp.not11 = icmp eq i64 %and, 0
  br i1 %cmp.not11, label %while.end, label %while.body

while.body:                                       ; preds = %if.end, %entry
  %c_new.016 = phi i64 [ %c_new.1, %if.end ], [ 0, %entry ]
  %g_new.015 = phi i64 [ %g_new.1, %if.end ], [ 1, %entry ]
  %c.014 = phi i64 [ %mul4, %if.end ], [ 1, %entry ]
  %g.013 = phi i64 [ %mul5, %if.end ], [ 2806196910506780709, %entry ]
  %n.addr.012 = phi i64 [ %shr, %if.end ], [ %and, %entry ]
  %and1 = and i64 %n.addr.012, 1
  %tobool.not = icmp eq i64 %and1, 0
  br i1 %tobool.not, label %if.end, label %if.then

if.then:                                          ; preds = %while.body
  %mul = mul i64 %g_new.015, %g.013
  %mul2 = mul i64 %c_new.016, %g.013
  %add = add i64 %mul2, %c.014
  br label %if.end

if.end:                                           ; preds = %if.then, %while.body
  %g_new.1 = phi i64 [ %mul, %if.then ], [ %g_new.015, %while.body ]
  %c_new.1 = phi i64 [ %add, %if.then ], [ %c_new.016, %while.body ]
  %add3 = add i64 %g.013, 1
  %mul4 = mul i64 %c.014, %add3
  %mul5 = mul i64 %g.013, %g.013
  %shr = lshr i64 %n.addr.012, 1
  %cmp.not = icmp samesign ult i64 %n.addr.012, 2
  br i1 %cmp.not, label %while.end, label %while.body, !llvm.loop !16

while.end:                                        ; preds = %if.end, %entry
  %g_new.0.lcssa = phi i64 [ 1, %entry ], [ %g_new.1, %if.end ]
  %c_new.0.lcssa = phi i64 [ 0, %entry ], [ %c_new.1, %if.end ]
  %mul6 = mul i64 %g_new.0.lcssa, %seed
  %add7 = add i64 %mul6, %c_new.0.lcssa
